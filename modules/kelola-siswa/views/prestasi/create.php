<?php
/**
 * Tambah Prestasi Siswa - Portal BIP
 */
?>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 space-y-6">
        <div>
            <h1 class="text-xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2.5">
                <span class="text-2xl">🏆</span> Catat Prestasi & Penghargaan Siswa
            </h1>
            <p class="text-xs text-slate-500 mt-1">Masukkan data prestasi, lomba, kejuaraan, atau penghargaan yang diraih oleh peserta didik.</p>
        </div>

        <form action="<?= url('kelola-siswa/prestasi/store') ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <?= CSRF::field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nama_prestasi" required placeholder="Contoh: Juara 1 Olimpiade Sains Nasional" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5">Peringkat / Juara <span class="text-rose-500">*</span></label>
                    <input type="text" name="peringkat" required placeholder="Contoh: Juara 1 / Medali Emas / Harapan 1" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="bidang" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                        ...
                        <option value="Bahasa / Debat">Bahasa & Debat</option>
                ...
                <div>
                    ...
                    <select name="tingkat" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                        <option value="Sekolah/Internal">Sekolah / Internal</option>
                        <option value="Kecamatan">Kecamatan</option>
                        <option value="Kota/Kabupaten" selected>Kota / Kabupaten</option>
                ...
                <div>
                    ...
                    <input type="text" name="tahun" value="<?= date('Y') ?>" maxlength="4" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="penyelenggara" placeholder="Contoh: Dinas Pendidikan Kota Palu" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5">Tanggal Perolehan</label>
                    <input type="date" name="tanggal_peroleh" value="<?= date('Y-m-d') ?>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5">Guru Pembimbing / Pendamping</label>
                    <input type="text" name="guru_pendamping" placeholder="Nama guru / pelatih" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nomor_sertifikat" placeholder="Nomor piagam jika ada" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1.5">Upload Scan Piagam / Sertifikat (PDF/JPG/PNG)</label>
            ...
            <div>
                ...
                <textarea name="keterangan" rows="3" placeholder="Informasi tambahan terkait kejuaraan, medali, atau apresiasi..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:bg-white focus:border-amber-500 transition-colors"></textarea>
            </div>
