...
$prefillMapel = trim($_GET['mapel'] ?? '');
$prefillKelas = trim($_GET['kelas'] ?? '');
$prefillTipe = trim($_GET['tipe'] ?? 'mapel');
?>
<div class="space-y-6">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight mt-1.5">Buat Wadah Grup Absen</h1>
            <p class="text-xs sm:text-sm text-slate-500">Konfigurasikan grup presensi siswa untuk rombongan belajar atau mata pelajaran</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="<?= url('kelola-absen-siswa/mapel') ?>" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs sm:text-sm shadow-sm transition-all">
                ...
                <span>Kembali ke Input Absen Mapel</span>
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
        <form action="<?= url('kelola-absen-siswa/group/store') ?>" method="POST" class="p-6 sm:p-8 space-y-8">
            ...
            <div class="pt-2 border-t border-slate-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="tahun_akademik_id" class="w-full px-3 py-2 rounded-2xl border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/50" required>
                                <?php foreach ($tahunList as $th): ?>
                                    <option value="<?= $th['id'] ?>"><?= htmlspecialchars($th['nama_tahun'] ?? '2026/2027') ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <select name="kelas" class="w-full px-3 py-2.5 rounded-2xl border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/50" required>
                            ...
                            <?php foreach ($kelasList as $k): 
                                $cleanK = preg_replace('/^Kelas\s+/i', '', $k['kelas']);
                                $isSel = ($prefillKelas === $cleanK || $prefillKelas === $k['kelas']);
                            ?>
                                <option value="<?= htmlspecialchars($cleanK) ?>" <?= $isSel ? 'selected' : '' ?>>Kelas <?= htmlspecialchars($k['kelas']) ?> (<?= htmlspecialchars($k['jenjang']) ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <!-- Mata Pelajaran -->
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Mata Pelajaran (Opsional jika Presensi Kelas)</label>
                        <input type="text" name="mata_pelajaran" value="<?= htmlspecialchars($prefillMapel) ?>" class="w-full px-4 py-2 rounded-2xl border border-slate-200 text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/50" placeholder="Cth: Matematika, IPAS, Bahasa Arab">
                    </div>

                    <!-- Guru Pengampu -->
                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Guru Pengampu / Wali Kelas (Opsional)</label>
                        <select name="guru_id" class="w-full px-3 py-2.5 rounded-2xl border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/50">
                            ...
                            <?php foreach ($guruList as $g): ?>
                                <option value="<?= $g['id'] ?>" <?= ($prefillGuru == $g['id']) ? 'selected' : '' ?>><?= htmlspecialchars($g['nama']) ?><?= !empty($g['gelar']) ? ', ' . htmlspecialchars($g['gelar']) : '' ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="pt-2 border-t border-slate-100">
                ...
                <div class="space-y-4 max-w-2xl">
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Judul Wadah Grup Absen <span class="text-slate-400 font-normal">(Kosongkan untuk otomatis)</span></label>
                        <input type="text" name="judul" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/50" placeholder="Cth: Presensi SD IPAS Kelas 3 Abdurrahman - Sem Ganjil">
                    </div>
                    <div>
