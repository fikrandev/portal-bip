...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Buat Kalender Pendidikan (Kaldik)</h1>
            <p class="text-xs sm:text-sm text-slate-500">Pilih unit sekolah dan susun agenda akademik tahunan</p>
        </div>
    ...
    <form method="POST" action="<?= url('kelola-perangkat-pembelajaran/kaldik/store') ?>" enctype="multipart/form-data" class="space-y-6">
        <?= CSRF::field() ?>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-5">
            ...
            <div class="pt-2">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <?php 
                    $selectedUnit = old('unit', $_GET['unit'] ?? ($teacherUnit ?? 'SD'));
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                <div class="md:col-span-3">
                    ...
                    <input type="text" name="judul" required placeholder="Contoh: Kalender Pendidikan SD TP 2026/2027 Semester Ganjil" value="<?= old('judul') ?>" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/50">
                </div>
            ...
            <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <label class="flex items-start gap-3 p-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 cursor-pointer">
                        <input type="radio" name="is_active" value="0" class="mt-0.5 text-slate-600 focus:ring-slate-500">
                        <div>
                            <span class="text-xs font-bold text-slate-700">
                                ⚪ Non-Aktif / Arsip / Persiapan
                            </span>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="2" placeholder="Catatan kebijakan kalender pendidikan, pedoman dinas pendidikan, dsb..." class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/50"></textarea>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs" id="tabel-agenda">
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium" id="agenda-container">
                        <?php
                        $defaultAgendas = [
                            ['mulai' => date('Y-07-15'), 'selesai' => date('Y-07-17'), 'nama' => 'Masa Pengenalan Lingkungan Sekolah (MPLS)', 'kategori' => 'kegiatan', 'ket' => 'Siswa Baru'],
                            ['mulai' => date('Y-07-18'), 'selesai' => date('Y-07-18'), 'nama' => 'Hari Pertama Efektif KBM Semester Ganjil', 'kategori' => 'kbm', 'ket' => 'Seluruh Siswa'],
                            ['mulai' => date('Y-08-17'), 'selesai' => date('Y-08-17'), 'nama' => 'HUT Kemerdekaan RI Ke-81', 'kategori' => 'libur_nasional', 'ket' => 'Upacara Bendera'],
                            ['mulai' => date('Y-09-20'), 'selesai' => date('Y-09-25'), 'nama' => 'Sumatif Tengah Semester (STS) Ganjil', 'kategori' => 'penilaian', 'ket' => 'Tes Tertulis & Praktik'],
                            ['mulai' => date('Y-12-05'), 'selesai' => date('Y-12-12'), 'nama' => 'Sumatif Akhir Semester (SAS) Ganjil', 'kategori' => 'penilaian', 'ket' => 'Evaluasi Akhir'],
                            ['mulai' => date('Y-12-20'), 'selesai' => date('Y-12-20'), 'nama' => 'Pembagian Rapor Semester Ganjil', 'kategori' => 'kegiatan', 'ket' => 'Pengambilan oleh Ortu'],
                            ['mulai' => date('Y-12-22'), 'selesai' => date('Y-12-31'), 'nama' => 'Libur Akhir Semester Ganjil', 'kategori' => 'libur_semester', 'ket' => 'Libur Siswa']
                        ];
                        foreach ($defaultAgendas as $idx => $def): ?>
                            <tr class="agenda-row">
                                <td class="py-2 px-2">
                                    <input type="date" name="agenda_tgl_mulai[]" value="<?= $def['mulai'] ?>" class="w-full px-2.5 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                </td>
                                <td class="py-2 px-2">
                                    <input type="date" name="agenda_tgl_selesai[]" value="<?= $def['selesai'] ?>" class="w-full px-2.5 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                </td>
                                <td class="py-2 px-2">
                                    <input type="text" name="agenda_kegiatan[]" value="<?= e($def['nama']) ?>" required placeholder="Nama kegiatan..." class="w-full px-2.5 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                </td>
                                <td class="py-2 px-2">
                                    <select name="agenda_kategori[]" class="w-full px-2 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                        <option value="kbm" <?= $def['kategori'] === 'kbm' ? 'selected' : '' ?>>KBM Efektif</option>
                                        <option value="penilaian" <?= $def['kategori'] === 'penilaian' ? 'selected' : '' ?>>Penilaian / Ujian</option>
                                        <option value="libur_nasional" <?= $def['kategori'] === 'libur_nasional' ? 'selected' : '' ?>>Libur Nasional</option>
                                        <option value="libur_semester" <?= $def['kategori'] === 'libur_semester' ? 'selected' : '' ?>>Libur Semester</option>
                                        <option value="kegiatan" <?= $def['kategori'] === 'kegiatan' ? 'selected' : '' ?>>Kegiatan Sekolah</option>
                                    </select>
                                </td>
                                <td class="py-2 px-2">
                                    <input type="text" name="agenda_pengecualian[]" value="" placeholder="Misal: 6, 9" class="w-full px-2.5 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                </td>
                                <td class="py-2 px-2">
                                    <input type="text" name="agenda_keterangan[]" value="<?= e($def['ket']) ?>" placeholder="Keterangan..." class="w-full px-2.5 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-emerald-500">
                                </td>
