 ...
 * 1. Elemen | 2. Capaian Pembelajaran | 3. Tujuan Pembelajaran | 4. KKTP | 5. Bulan
 */
$currentGuru = $currentGuruId ?? 0;
        ...
        <div>
            <div class="flex items-center gap-2">
                <span class="px-3 py-1 rounded-xl text-xs font-black bg-indigo-50 text-indigo-700 border border-indigo-200">
                    Grup Tahun Ajaran <?= e($active_ta_name ?? '2026/2027') ?>
                </span>
            </div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight mt-1.5">Input Dokumen CP & ATP</h1>
            <p class="text-xs sm:text-sm text-slate-500">Format 5 Kolom: Elemen, Capaian Pembelajaran, Tujuan Pembelajaran, KKTP, dan Alokasi Bulan</p>
        </div>
        <a href="<?= url('kelola-perangkat-pembelajaran/cpatp') ?>" class="px-4 py-2 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition-colors self-start">
            &larr; Kembali
        </a>
    </div>

    <!-- Form Card -->
    <form method="POST" action="<?= url("kelola-perangkat-pembelajaran/cpatp/store/{$group['id']}") ?>" enctype="multipart/form-data" class="space-y-6">
        <?= CSRF::field() ?>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-4">
            ...
            <input type="hidden" name="unit" value="<?= e($group['unit']) ?>">
            <input type="hidden" name="tahun_akademik_id" value="<?= e($group['tahun_akademik_id']) ?>">
            <input type="hidden" name="semester" value="<?= e($group['semester']) ?>">
                ...
                <div>
                    ...
                    <div class="px-4 py-2.5 rounded-2xl border border-slate-100 bg-slate-50 text-xs font-bold text-slate-700">
                        ...
                        foreach($ta_list as $ta) {
                            if ($ta['id'] == $group['tahun_akademik_id']) { $taName = $ta['nama_tahun']; break; }
                        }
                        echo e($taName);
                        ?>
                    </div>
            ...
            <div class="border-t border-slate-100 pt-4">
                ...
                $selected_guru_id = old('guru_id', $currentGuruId);
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-100">
                <div class="sm:col-span-2">
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Judul Materi Pokok / Dokumen <span class="text-rose-500">*</span></label>
                    <input type="text" name="judul" required placeholder="Contoh: Bab 1: Bilangan Bulat" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-bold text-indigo-900 focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white shadow-sm">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Mata Pelajaran <span class="text-rose-500">*</span></label>
                    <select name="mata_pelajaran" id="mata_pelajaran" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50" required disabled>
                ...
                <div>
                    ...
                    <select name="tingkat_kelas" id="tingkat_kelas" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50" required disabled>
                        <option value="">Pilih Guru Dahulu</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="fase" id="fase" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50">
                        ...
                        <option value="Fase E">Fase E (SMA Kls 10)</option>
                        <option value="Fase F">Fase F (SMA Kls 11-12)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="alokasi_waktu_input" name="alokasi_waktu" placeholder="Contoh: 12" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50">
                </div>
            </div>
        </div>

        <!-- Tabel CP & ATP (5 Kolom: Elemen, Capaian Pembelajaran, Tujuan Pembelajaran, KKTP, Bulan) -->
        <div class="space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Input Elemen, Capaian Pembelajaran, Tujuan Pembelajaran, KKTP, dan Alokasi Bulan</p>
                </div>
            ...
            <div id="cpatp-blocks-container" class="space-y-5">
                ...
                <div class="cpatp-block bg-white rounded-3xl p-6 border-2 border-slate-200/80 shadow-sm space-y-4 relative" data-block-idx="0">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <span class="px-3 py-1 rounded-xl text-xs font-black bg-indigo-50 text-indigo-700 border border-indigo-200 block-badge">
                            Blok Elemen & TP #1
                        </span>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                        ...
                        <div class="lg:col-span-3">
                            ...
                            <input type="text" name="row_elemen[]" required placeholder="Contoh: Bilangan" class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50">
                        </div>

                        <!-- Kolom 2: Capaian Pembelajaran (CP) -->
                        <div class="lg:col-span-5">
                            ...
                            <textarea name="row_cp[]" rows="3" required placeholder="Contoh: Peserta didik menunjukkan pemahaman dan memiliki intuisi bilangan (number sense)..." class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs leading-relaxed focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50"></textarea>
                        </div>

                        <!-- Kolom 3: Tujuan Pembelajaran (TP) -->
                        <div class="lg:col-span-4">
                            ...
                            <textarea name="row_tp[]" rows="3" required placeholder="Contoh: 2.1 Peserta didik dapat menunjukkan pemahaman tentang bilangan cacah sampai 100" class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs leading-relaxed focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50"></textarea>
                        </div>
                    </div>

                    <!-- Sub-Tabel Kolom 4 (KKTP) & Kolom 5 (Bulan) -->
                    <div class="pt-2 border-t border-slate-100">
                        ...
                        <div class="space-y-2 kktp-list-container">
                            ...
                            <div class="flex items-center gap-2 kktp-row">
                                <div class="flex-1">
                                    <input type="text" name="row_kktp[0][]" placeholder="Contoh: 2.1.1 Membaca dan menulis bilangan cacah sampai 100" class="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/30">
                                </div>
                                <div class="w-32">
        ...
        <div class="flex items-center justify-end gap-3 pt-2">
            <a href="<?= url('kelola-perangkat-pembelajaran/cpatp') ?>" class="px-5 py-2.5 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition-colors">
                Batal
            </a>
            <button type="submit" name="draft" value="1" class="px-5 py-2.5 rounded-2xl bg-slate-800 hover:bg-slate-900 text-white font-semibold text-xs shadow-md transition-all">
                Simpan sebagai Draft
            </button>
            <button type="submit" name="ajukan" value="1" class="px-6 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-md shadow-indigo-500/20 transition-all">
...
<!-- Data Mapping JSON Penugasan Guru & Script Dinamis -->
<script>
const BULAN_OPTIONS = <?= json_encode($bulanOptions) ?>;
const GROUP_UNIT = <?= json_encode($group['unit'] ?? 'SD') ?>;
const GROUP_SEMESTER = <?= json_encode($group['semester'] ?? 'Ganjil') ?>;
...
function addBlockRow() {
    ...
    let bulanOptionsHtml = '';
    BULAN_OPTIONS.forEach(b => {
        bulanOptionsHtml += `<option value="${b}">${b}</option>`;
    });

    div.innerHTML = `
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <span class="px-3 py-1 rounded-xl text-xs font-black bg-indigo-50 text-indigo-700 border border-indigo-200 block-badge">
                Blok Elemen & TP #${blockIdx + 1}
            </span>
            <button type="button" onclick="removeBlockRow(this)" class="p-1.5 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-100 transition-colors" title="Hapus Blok Ini">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
            </button>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-5">
                <label class="block text-xs font-bold text-slate-700 uppercase mb-1">2. Capaian Pembelajaran <span class="text-rose-500">*</span></label>
                <textarea name="row_cp[]" rows="3" required placeholder="Deskripsi Capaian Pembelajaran Elemen..." class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs leading-relaxed focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50"></textarea>
            </div>
            <div class="lg:col-span-4">
                <label class="block text-xs font-bold text-slate-700 uppercase mb-1">3. Tujuan Pembelajaran <span class="text-rose-500">*</span></label>
                <textarea name="row_tp[]" rows="3" required placeholder="Deskripsi Tujuan Pembelajaran (TP)..." class="w-full px-3.5 py-2.5 rounded-2xl border border-slate-200 text-xs leading-relaxed focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50"></textarea>
            </div>
        </div>
        ...
        <div class="pt-2 border-t border-slate-100">
            ...
            <div class="space-y-2 kktp-list-container">
                <div class="flex items-center gap-2 kktp-row">
                    <div class="flex-1">
                        <input type="text" name="row_kktp[${blockIdx}][]" placeholder="Deskripsi KKTP / Indikator..." class="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/30">
                    </div>
                    <div class="w-32">
                        <select name="row_bulan[${blockIdx}][]" onchange="onBulanChanged(this)" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50">
                            ${bulanOptionsHtml}
                        </select>
                    </div>
                    <div class="w-28">
                        <select name="row_pekan[${blockIdx}][]" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50">
...
function removeBlockRow(btn) {
    const container = document.getElementById('cpatp-blocks-container');
    if (container.querySelectorAll('.cpatp-block').length > 1) {
        btn.closest('.cpatp-block').remove();
        refreshBlockIndices();
    } else {
        alert('Minimal harus ada 1 blok Elemen & TP.');
    }
}

function refreshBlockIndices() {
    const blocks = document.querySelectorAll('#cpatp-blocks-container .cpatp-block');
    blocks.forEach((block, bIdx) => {
        ...
        block.querySelectorAll('.kktp-row').forEach(row => {
            const kktpInput = row.querySelector('input[name^="row_kktp"]');
            const bulanSelect = row.querySelector('select[name^="row_bulan"]');
            const pekanSelect = row.querySelector('select[name^="row_pekan"]');
            if (kktpInput) kktpInput.name = `row_kktp[${bIdx}][]`;
            if (bulanSelect) bulanSelect.name = `row_bulan[${bIdx}][]`;
            if (pekanSelect) pekanSelect.name = `row_pekan[${bIdx}][]`;
        });
    });
}
...
function addKktpRow(btn) {
    ...
    BULAN_OPTIONS.forEach(b => {
        ...
        bulanOptionsHtml += `<option value="${b}" ${selected}>${b}</option>`;
    });
    ...
    div.innerHTML = `
        <div class="flex-1">
            <input type="text" name="row_kktp[${bIdx}][]" placeholder="Deskripsi KKTP / Indikator..." class="w-full px-3.5 py-2 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50/30">
        </div>
        <div class="w-32">
        ...
        <div class="w-28">
            <select name="row_pekan[${bIdx}][]" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-none bg-slate-50">
...
function removeKktpRow(btn) {
    ...
    if (container.querySelectorAll('.kktp-row').length > 1) {
        btn.closest('.kktp-row').remove();
    } else {
        alert('Minimal harus ada 1 baris KKTP pada blok ini.');
    }
}
</script>
...
function onMapelChanged() {
    ...
    // Reset data jadwal lama agar tidak tercampur tanggal mapel/kelas sebelumnya
    jadwalData = { hari_mengajar: [], tahun_akademik: null, heb_data: [] };
    refreshAllPekanDropdowns();
    ...
    // Filter penugasanData based on selected mapel
    ...
    // Jika hanya ada 1 kelas untuk mapel ini, langsung otomatis pilih dan ambil jadwal
...
function fetchPenugasan(guruId) {
    ...
    fetch(`<?= url('kelola-perangkat-pembelajaran/cpatp/ajax-penugasan') ?>/${guruId}`)
        ...
        .then(data => {
            ...
            mapels.forEach(mapel => { mapelSelect.innerHTML += `<option value="${mapel}">${mapel}</option>`; });

            kelasSelect.innerHTML = '<option value="">Pilih Mapel Dahulu</option>';
            mapelSelect.disabled = false;
            // Kelas stays disabled until mapel is selected
        })
        .catch(err => {
            console.error(err);
