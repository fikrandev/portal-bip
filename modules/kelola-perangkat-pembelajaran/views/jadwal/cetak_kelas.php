...
<head>
    ...
    <title>Jadwal Pelajaran Kelas <?= e($namaKelas) ?> — <?= e($grup['tahun_ajaran']) ?></title>
    <style>
        @page {
            size: A4 landscape;
...
<body>
    ...
    <div class="no-print" style="background:#1e293b; color:#fff; padding:10px 20px; display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-radius:8px;">
        <div style="font-family:sans-serif; font-size:12px; font-weight:bold;">
            📄 Cetak Jadwal Pelajaran: Kelas <?= e($namaKelas) ?> (<?= e($grup['tahun_ajaran']) ?>)
        </div>
        <div>
    ...
    <div class="header">
        ...
        <h2>JADWAL PELAJARAN TAHUN AJARAN <?= strtoupper(e($grup['tahun_ajaran'])) ?></h2>
        <p>Unit: <?= e($grup['jenjang']) ?> IT Bina Insan Palu | Semester: <?= e($grup['semester']) ?></p>
    </div>

    <div class="title-box">
        <h3>ROMBONGAN BELAJAR: <?= strtoupper(e($namaKelas)) ?></h3>
                ...
                <?php foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $day): ?>
                    ...
                    <td>
                        <?php if ($item): ?>
                            ...
                            <div class="teacher-box"><?= e($item['nama_guru']) ?></div>
