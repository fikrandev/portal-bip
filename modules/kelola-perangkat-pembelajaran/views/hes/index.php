...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Hari Efektif Sekolah (HES)</h1>
    ...
    <div class="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-sm space-y-4">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1 text-xs">
            ...
            <a href="<?= url('kelola-perangkat-pembelajaran/hes?' . http_build_query(array_merge($_GET, ['unit' => '']))) ?>" class="px-3.5 py-1.5 rounded-xl font-semibold transition-all whitespace-nowrap <?= empty($filter_unit) ? 'bg-slate-800 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' ?>">
                🌐 Semua Unit
            </a>
        ...
        <form method="GET" action="<?= url('kelola-perangkat-pembelajaran/hes') ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2 border-t border-slate-100">
            ...
            <table class="w-full text-left text-xs">
                <thead>
                    <tr class="bg-slate-50 text-slate-500 uppercase tracking-wider text-[10px] border-b border-slate-200">
                        ...
                        <th class="py-3.5 px-4 font-bold">Judul HES</th>
                        <th class="py-3.5 px-4 font-bold">Tahun / Semester</th>
                        <th class="py-3.5 px-4 font-bold">Total Hari Efektif</th>
                        ...
                        <tr>
                            <td colspan="7" class="py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-600">Belum ada data Hari Efektif Sekolah <?= !empty($filter_unit) ? "untuk Unit {$filter_unit}" : '' ?></p>
                                    <p class="text-xs text-slate-400">Klik tombol "Buat HES Baru" di atas untuk menambahkan data.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($items as $row): ?>
                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors">
                                ...
                                <td class="py-3.5 px-4">
                                    ...
                                    <div class="text-[10px] text-slate-400">Dibuat: <?= date('d M Y', strtotime($row['created_at'])) ?></div>
                                </td>
                                <td class="py-3.5 px-4 whitespace-nowrap">
                                    <div class="font-semibold text-slate-800"><?= e($row['nama_tahun'] ?? 'Tahun Aktif') ?></div>
                                ...
                                <td class="py-3.5 px-4 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <form method="POST" action="<?= url("kelola-perangkat-pembelajaran/delete/{$row['id']}") ?>" onsubmit="return confirm('Apakah Anda yakin ingin menghapus dokumen HES ini?');" class="inline">
                                            <?= CSRF::field() ?>
