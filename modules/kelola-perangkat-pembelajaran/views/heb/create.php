...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Buat Hari Efektif Belajar (HEB)</h1>
            <p class="text-xs sm:text-sm text-slate-500">Hitung pekan efektif KBM & alokasi Jam Pelajaran (JP) per mata pelajaran berdasarkan Kaldik Unit</p>
        </div>
    ...
    <form method="POST" action="<?= url('kelola-perangkat-pembelajaran/heb/store') ?>" enctype="multipart/form-data" class="space-y-6">
        <?= CSRF::field() ?>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-5">
            ...
            <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
                ...
                <div id="kaldik-info-box" class="hidden p-3 rounded-xl bg-cyan-50/80 border border-cyan-200 text-xs text-cyan-900 flex items-center justify-between">
                    <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
                <div class="lg:col-span-3">
                    ...
                    <input type="text" name="judul" required placeholder="Contoh: Rincian Pekan Efektif & Alokasi Waktu Matematika Kelas X" value="<?= old('judul') ?>" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Mata Pelajaran <span class="text-rose-500">*</span></label>
                    <input type="text" name="mata_pelajaran" required placeholder="Contoh: Matematika, IPA, PAI..." value="<?= old('mata_pelajaran') ?>" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Tingkat / Kelas <span class="text-rose-500">*</span></label>
                    <input type="text" name="tingkat_kelas" required placeholder="Contoh: Kelas VII / Kelas X IPA" value="<?= old('tingkat_kelas') ?>" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                </div>
                ...
                <div>
                    ...
                    <select name="fase" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                        ...
                        <option value="Fondasi (PAUD)">Fase Fondasi (PAUD / TK)</option>
                ...
                <div>
                    ...
                    <select name="semester" id="semester-heb" onchange="onSemesterChanged(this.value)" required class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-medium focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                ...
                <div class="sm:col-span-2 lg:col-span-3">
                    ...
                    <input type="number" name="jp_per_minggu" id="jp_per_minggu" value="3" min="1" max="10" oninput="hitungHEB()" required class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-bold text-cyan-800 focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs" id="tabel-heb">
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium" id="heb-body">
                        <?php
                        $defaultPekanGanjil = [
                            ['nama' => 'Juli', 'total' => 5, 'non' => 2, 'ket' => 'Libur Semester & MPLS'],
                            ['nama' => 'Agustus', 'total' => 4, 'non' => 0, 'ket' => 'KBM Efektif Penuh'],
                            ['nama' => 'September', 'total' => 4, 'non' => 1, 'ket' => 'Sumatif Tengah Semester'],
                            ['nama' => 'Oktober', 'total' => 5, 'non' => 0, 'ket' => 'KBM Efektif Penuh'],
                            ['nama' => 'November', 'total' => 4, 'non' => 0, 'ket' => 'KBM Efektif Penuh'],
                            ['nama' => 'Desember', 'total' => 5, 'non' => 3, 'ket' => 'SAS & Libur Semester']
                        ];
                        foreach ($defaultPekanGanjil as $i => $b):
                            $efektif = max(0, $b['total'] - $b['non']);
                        ?>
                            <tr class="heb-row">
                                ...
                                <td class="py-2.5 px-4">
                                    <input type="number" name="pekan_total[]" value="<?= $b['total'] ?>" min="0" max="6" oninput="hitungHEB()" class="pekan-total-input w-24 px-3 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 text-center font-bold focus:outline-none focus:ring-1 focus:ring-cyan-500">
                                </td>
                                <td class="py-2.5 px-4">
                                    <input type="number" name="pekan_non_efektif[]" value="<?= $b['non'] ?>" min="0" max="6" oninput="hitungHEB()" class="pekan-non-input w-24 px-3 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 text-center font-bold focus:outline-none focus:ring-1 focus:ring-cyan-500">
                                </td>
                                <td class="py-2.5 px-4 font-bold text-cyan-700">
                                    <input type="number" name="pekan_efektif[]" value="<?= $efektif ?>" readonly class="pekan-efektif-input w-24 px-3 py-1.5 rounded-xl border border-cyan-200 text-xs bg-cyan-50 text-center font-extrabold text-cyan-800 focus:outline-none">
                                </td>
                                <td class="py-2.5 px-4">
                                    <input type="text" name="pekan_ket[]" value="<?= e($b['ket']) ?>" placeholder="Rincian agenda..." class="w-full px-3 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-cyan-500">
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                    <tfoot>
                        <tr class="bg-cyan-50/70 font-extrabold text-cyan-950 border-t-2 border-cyan-600">
                            ...
                            <td class="py-3 px-4 text-center text-sm" id="sum-pekan-total">27</td>
                            <td class="py-3 px-4 text-center text-sm text-rose-700" id="sum-pekan-non">6</td>
                            <td class="py-3 px-4 text-center text-base text-cyan-800" id="sum-pekan-efektif">21</td>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-4">
            ...
            <div class="p-5 rounded-2xl bg-gradient-to-br from-cyan-50 to-teal-50 border border-cyan-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-cyan-950 font-medium mt-0.5">
                        <span id="label-formula-pekan" class="font-bold text-cyan-700">21 Pekan</span> x 
                        <span id="label-formula-jp" class="font-bold text-cyan-700">3 JP/Minggu</span> = 
                        <span id="label-formula-total" class="text-xl font-extrabold text-cyan-900 ml-1">63 JP</span>
                    </p>
                </div>
                <div class="px-5 py-2.5 rounded-2xl bg-white text-cyan-900 border border-cyan-200 shadow-sm text-center">
                    ...
                    <p class="text-2xl font-black text-cyan-700" id="total-jp-display">63 JP</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                <div>
                    ...
                    <input type="number" name="jp_kbm" id="jp_kbm" value="54" min="0" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-bold text-slate-800 focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">JP Asesmen / Penilaian (STS & SAS)</label>
                    <input type="number" name="jp_penilaian" id="jp_penilaian" value="6" min="0" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-bold text-slate-800 focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">JP Cadangan / Pengayaan</label>
                    <input type="number" name="jp_cadangan" id="jp_cadangan" value="3" min="0" class="w-full px-4 py-2.5 rounded-2xl border border-slate-200 text-xs font-bold text-slate-800 focus:ring-2 focus:ring-cyan-500 focus:outline-none bg-slate-50/50">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-4">
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-700 mb-1">Unggah Dokumen / Rincian HEB (PDF / Word / Excel / JPG)</label>
...
const pekanGenap = [
    { nama: 'Januari', total: 5, non: 1, ket: 'Tahun Baru & Awal KBM' },
    { nama: 'Februari', total: 4, non: 0, ket: 'KBM Efektif Penuh' },
    { nama: 'Maret', total: 4, non: 1, ket: 'Sumatif Tengah Semester' },
    { nama: 'April', total: 4, non: 2, ket: 'Libur Idul Fitri' },
    { nama: 'Mei', total: 5, non: 1, ket: 'KBM & Libur Waisak' },
    { nama: 'Juni', total: 4, non: 3, ket: 'SAS & Libur Kenaikan Kelas' }
];

function gantiBulanHeb(smt) {
    const list = smt === 'Genap' ? pekanGenap : pekanGanjil;
    const tbody = document.getElementById('heb-body');
    tbody.innerHTML = '';

    list.forEach((b, i) => {
        const ef = Math.max(0, b.total - b.non);
        const tr = document.createElement('tr');
        tr.className = 'heb-row';
        tr.innerHTML = `
            <td class="py-2.5 px-3 text-center text-slate-400">${i + 1}</td>
            <td class="py-2.5 px-4 font-bold text-slate-800">
                <input type="text" name="bulan_nama[]" value="${b.nama}" readonly class="bg-transparent font-bold text-slate-800 text-xs w-full focus:outline-none">
            ...
            <td class="py-2.5 px-4 font-bold text-cyan-700">
                <input type="number" name="pekan_efektif[]" value="${ef}" readonly class="pekan-efektif-input w-24 px-3 py-1.5 rounded-xl border border-cyan-200 text-xs bg-cyan-50 text-center font-extrabold text-cyan-800 focus:outline-none">
            </td>
            <td class="py-2.5 px-4">
                <input type="text" name="pekan_ket[]" value="${b.ket}" placeholder="Rincian agenda..." class="w-full px-3 py-1.5 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:outline-none focus:ring-1 focus:ring-cyan-500">
            </td>
        `;
