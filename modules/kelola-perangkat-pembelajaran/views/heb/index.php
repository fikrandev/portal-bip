...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 mb-2">
                <a href="<?= url('kelola-perangkat-pembelajaran/heb') ?>" class="px-3.5 py-1.5 rounded-xl bg-cyan-600 text-white font-bold text-xs shadow-sm">
            ...
            <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Hari Efektif Belajar (HEB)</h1>
            <p class="text-xs sm:text-sm text-slate-500">Perhitungan pekan efektif KBM dan alokasi Jam Pelajaran (JP) per mata pelajaran dan unit</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="<?= url('kelola-perangkat-pembelajaran/heb/create') ?>" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-cyan-600 hover:bg-cyan-700 text-white font-semibold text-xs sm:text-sm shadow-md shadow-cyan-500/20 transition-all">
    ...
    <div class="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-sm space-y-4">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1 text-xs">
            ...
            <a href="<?= url('kelola-perangkat-pembelajaran/heb?' . http_build_query(array_merge($_GET, ['unit' => '']))) ?>" class="px-3.5 py-1.5 rounded-xl font-semibold transition-all whitespace-nowrap <?= empty($filter_unit) ? 'bg-slate-800 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' ?>">
                🌐 Semua Unit
            </a>
            <?php foreach ($unit_list as $uKey => $uInfo): ?>
                <?php $isActiveUnit = ($filter_unit === $uKey); ?>
                <a href="<?= url('kelola-perangkat-pembelajaran/heb?' . http_build_query(array_merge($_GET, ['unit' => $uKey]))) ?>" class="px-3.5 py-1.5 rounded-xl font-semibold transition-all whitespace-nowrap inline-flex items-center gap-1.5 <?= $isActiveUnit ? 'bg-cyan-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' ?>">
        ...
        <form method="GET" action="<?= url('kelola-perangkat-pembelajaran/heb') ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2 border-t border-slate-100">
            ...
            <div>
                ...
                <select name="ta" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:ring-2 focus:ring-cyan-500 focus:outline-none">
            ...
            <div>
                ...
                <select name="semester" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:ring-2 focus:ring-cyan-500 focus:outline-none">
            ...
            <div>
                ...
                <select name="status" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:ring-2 focus:ring-cyan-500 focus:outline-none">
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    <input type="text" name="search" value="<?= e($search) ?>" placeholder="Cari mapel / kelas..." class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs bg-slate-50 focus:ring-2 focus:ring-cyan-500 focus:outline-none">
                    <button type="submit" class="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-semibold rounded-xl text-xs transition-colors">
                        Cari
                    </button>
                </div>
            </div>
            ...
            <table class="w-full text-left text-xs">
                <thead>
                    <tr class="bg-slate-50 text-slate-500 uppercase tracking-wider text-[10px] border-b border-slate-200">
                        ...
                        <th class="py-3.5 px-4 font-bold">Total JP Efektif</th>
                        ...
                        <tr>
                            <td colspan="7" class="py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-600">Belum ada data Hari Efektif Belajar <?= !empty($filter_unit) ? "untuk Unit {$filter_unit}" : '' ?></p>
                                    <p class="text-xs text-slate-400">Klik tombol "Buat HEB Baru" di atas untuk menambahkan data.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($items as $row): ?>
                            ...
                            <tr class="hover:bg-slate-50/70 transition-colors">
                                ...
                                <td class="py-3.5 px-4">
                                    ...
                                    <div class="text-[11px] text-cyan-700 font-bold"><?= e($row['mata_pelajaran']) ?></div>
                                </td>
                                <td class="py-3.5 px-4 whitespace-nowrap">
                                    <div class="font-semibold text-slate-800"><?= e($row['tingkat_kelas']) ?></div>
                                    <div class="text-[10px] text-slate-400"><?= !empty($row['fase']) ? 'Fase ' . e($row['fase']) : 'Semester ' . e($row['semester']) ?></div>
                                </td>
                                <td class="py-3.5 px-4 whitespace-nowrap">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-cyan-50 text-cyan-800 font-extrabold text-xs">
                                ...
                                <td class="py-3.5 px-4 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="<?= url("kelola-perangkat-pembelajaran/heb/detail/{$row['id']}") ?>" class="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors" title="Lihat Detail">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /></svg>
                                        </a>
                                        <a href="<?= url("kelola-perangkat-pembelajaran/heb/edit/{$row['id']}") ?>" class="p-1.5 rounded-lg bg-cyan-50 hover:bg-cyan-100 text-cyan-700 transition-colors" title="Edit HEB">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                                        </a>
                                        <a href="<?= url("kelola-perangkat-pembelajaran/heb/cetak/{$row['id']}") ?>" target="_blank" class="p-1.5 rounded-lg bg-sky-50 hover:bg-sky-100 text-sky-700 transition-colors" title="Cetak Dokumen">
                                        ...
                                        <form method="POST" action="<?= url("kelola-perangkat-pembelajaran/delete/{$row['id']}") ?>" onsubmit="return confirm('Apakah Anda yakin ingin menghapus dokumen HEB ini?');" class="inline">
                                            <?= CSRF::field() ?>
            ...
            <div class="px-5 py-4 border-t border-slate-100 flex items-center justify-between">
                ...
                <div class="flex gap-1">
                    <?php for ($p = 1; $p <= $totalPages; $p++): ?>
                        <a href="<?= url('kelola-perangkat-pembelajaran/heb?' . http_build_query(array_merge($_GET, ['page' => $p]))) ?>" class="px-3 py-1 rounded-lg text-xs font-semibold <?= $p === $page ? 'bg-cyan-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' ?>">
                            <?= $p ?>
                        </a>
                    <?php endfor; ?>
                </div>
