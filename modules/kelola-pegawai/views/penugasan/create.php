<?php
/**
 * View Tambah Anggota Penugasan dalam Grup SK
 */
?>
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            <a href="<?= url('kelola-pegawai/penugasan/grup/' . $grup['id']) ?>" class="p-2 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl text-slate-600 transition-colors shadow-xs">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Tambah Anggota Penugasan</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Grup: <strong class="text-primary-700"><?= e($grup['nama_grup']) ?></strong> (Semester <?= e($grup['semester']) ?>)</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-primary-100/60 shadow-sm p-6 sm:p-8 relative">
        <form action="<?= url('kelola-pegawai/penugasan/grup/' . $grup['id'] . '/store') ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-5">
                ...
                <div class="relative">
                    ...
                    <select name="unit_tugas_id" id="select-unit-tugas" required class="searchable-select w-full" data-placeholder="-- Pilih Unit Tugas --" data-search-placeholder="Cari unit tugas..." onchange="onUnitChanged(this)">
                        ...
                        <?php foreach ($unitTugasList as $ut): ?>
                            <option value="<?= e($ut['id']) ?>" data-unit-name="<?= e($ut['nama']) ?>" <?= old('unit_tugas_id') == $ut['id'] ? 'selected' : '' ?>>
                                <?= e($ut['nama']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="relative">
                    ...
                    <select name="jabatan_id" id="select-jabatan" required class="searchable-select w-full" data-placeholder="-- Pilih Jabatan --" data-search-placeholder="Cari jabatan..." onchange="onJabatanChanged(this)">
                        ...
                        <?php foreach ($jabatanList as $jb): ?>
                            <option value="<?= e($jb['id']) ?>" data-nama="<?= strtolower(e($jb['nama'])) ?>" <?= old('jabatan_id') == $jb['id'] ? 'selected' : '' ?>>
                                <?= e($jb['nama']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <!-- Section Khusus Wali Kelas (Muncul jika Jabatan adalah Wali Kelas) -->
                <div id="section-wali-kelas" class="border border-amber-200 bg-amber-50/40 rounded-2xl p-4 sm:p-5 shadow-xs transition-all relative" style="display: none;">
                    ...
                    <div class="relative">
                        ...
                        <select name="wali_kelas_nama" id="select-wali-kelas" class="searchable-select w-full" data-placeholder="-- Pilih Kelas Siswa --" data-search-placeholder="Cari kelas (misal: Kelas 7A, 1B)...">
                            ...
                            <?php foreach ($kelasListWithUnit as $k): ?>
                                <option value="<?= e($k['nama_kelas']) ?>" data-unit="<?= e($k['unit']) ?>" <?= old('wali_kelas_nama') === $k['nama_kelas'] ? 'selected' : '' ?>>
                ...
                <div class="border border-indigo-100 bg-gradient-to-br from-indigo-50/40 via-white to-blue-50/30 rounded-2xl p-4 sm:p-5 shadow-xs transition-all relative">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-indigo-100/70 mb-4">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer select-none shrink-0">
                            <input type="checkbox" name="is_guru" id="toggle-is-guru" value="1" class="sr-only peer" onchange="toggleGuruSection(this.checked)">
                    ...
                    <div id="section-tugas-mengajar" class="space-y-3" style="display: none;">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-4 border-t border-indigo-100/70">
                            ...
                            <div class="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-indigo-200 text-xs font-bold text-slate-700 shadow-xs">
                                ...
                                <span id="badge-total-jp" class="px-2.5 py-0.5 rounded-lg bg-indigo-600 text-white font-extrabold text-xs">0 JP / Minggu</span>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
// Data referensi untuk Searchable Select Dropdown
