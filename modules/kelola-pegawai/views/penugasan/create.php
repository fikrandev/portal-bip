...
<div class="max-w-4xl mx-auto">
    ...
    <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Tambahkan data penugasan atau SK baru untuk pegawai.</p>
    </div>
    ...
    <div class="p-6">
        <form action="<?= url('kelola-pegawai/penugasan/store') ?>" method="POST" enctype="multipart/form-data">
            <?= CSRF::field() ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Pilih Pegawai <span class="text-rose-500">*</span></label>
                    <select name="pegawai_id" required class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors">
                        ...
                        <?php foreach ($pegawaiList as $p): ?>
                            <option value="<?= e($p['id']) ?>" <?= old('pegawai_id') == $p['id'] ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <input type="text" name="no_sk" value="<?= old('no_sk') ?>" required
                ...
                <div>
                    ...
                    <input type="date" name="tmt_mulai" value="<?= old('tmt_mulai') ?>" required
                           class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tst_selesai" value="<?= old('tst_selesai') ?>"
                           class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors">
                    <p class="text-[11px] text-slate-500 mt-1">Kosongkan jika penugasan tidak memiliki batas waktu (masih aktif).</p>
                </div>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Berkas SK (Opsional)</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="3" 
                              class="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors" 
                              placeholder="Catatan tambahan mengenai penugasan ini..."><?= old('keterangan') ?></textarea>
                </div>
